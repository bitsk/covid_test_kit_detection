...
class RetinaFace(BaseFaceDetector):
    ...
    def _postprocess(self, raw_prediction: Tuple[np.ndarray, np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # conf = np.load('./dump/conf_torch.npy')[0]
        # loc = np.load('./dump/loc_torch.npy')[0]
        # landms= np.load('./dump/landms_torch.npy')[0]
        # np.save('./dump/loc_openvino.npy', loc)
        # np.save('./dump/conf_openvino.npy', conf)
        # np.save('./dump/landms_openvino.npy', landms)
        ...
        scale1 = np.array([img_w, img_h, img_w, img_h, img_w, img_h, img_w, img_h], dtype=np.float32)
        resize_scale = np.array(self.resize_scale * 4, dtype=np.float32)
    ...
    def _predict_raw_batch(self, images: List[np.ndarray]):
        if len(images) > self.batch_size:
            raise NotImplementedError
        imgs = np.zeros(shape=(self.batch_size, 3, self.config["image_size"], self.config["image_size"]),
                        dtype=np.float32)
        ...
        pred = (pred[self.output_names[0]][:len(images)], pred[self.output_names[1]][:len(images)],
                pred[self.output_names[2]][:len(images)])
        return pred

    def _postprocess_batch(self, raw_predictions: List[Tuple[np.ndarray, np.ndarray, np.ndarray]]) -> List[
    ...
    def predict_batch(self, images: List[np.ndarray]) -> List[Tuple[np.ndarray, np.ndarray]]:
        ...
        for origin_size, boxes, landms in zip(origin_sizes, bboxes_batch, landms_batch):
            ...
            landmark_scale = np.array(
                [origin_size[1], origin_size[0], origin_size[1], origin_size[0], origin_size[1], origin_size[0],
                 origin_size[1], origin_size[0], origin_size[1], origin_size[0]], dtype=np.float32)
